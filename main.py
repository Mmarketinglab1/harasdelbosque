...
async def init_db_and_admin():
    try:
        ...
        with SessionLocal() as db:
            admin = db.query(models.Operator).filter(models.Operator.username == "admin_hara").first()
            if not admin:
                logger.info("Creando usuario administrador por defecto: admin_hara")
                new_admin = models.Operator(
                    username="admin_hara",
                    hashed_password=get_password_hash(os.getenv("ADMIN_HARA_PASS", "CHANGE_ME")),
                    full_name="Administrador Hara",
                    role="admin"
                )
                db.add(new_admin)
                db.commit()
                logger.info("Usuario admin_hara creado exitosamente.")
            else:
                logger.info("El usuario admin_hara ya existe.")
    except Exception as e:
        logger.error(f"Error crítico en la inicialización de la base de datos: {e}")
...
async def manual_init_db():
    await init_db_and_admin()
    return {"status": "Initialization triggered. Check logs for results."}
app.mount("/static", StaticFiles(directory="."), name="static")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

# --- SEGURIDAD ---
SECRET_KEY = os.getenv("SECRET_KEY", "REPLACE_THIS_WITH_ENV_VALUE")
ALGORITHM = "HS256"
...
@app.get("/api/bot/availability")
async def get_bot_availability():
    headers = {
        "apikey": SUPABASE_KEY,
    ...
    async with httpx.AsyncClient() as client:
        try:
            # Fetch rooms
            rooms_resp = await client.get(f"{SUPABASE_URL}/rest/v1/rooms?select=*", headers=headers)
            rooms = rooms_resp.json() if rooms_resp.status_code == 200 else []
            
            # Fetch reservations
            res_resp = await client.get(f"{SUPABASE_URL}/rest/v1/reservations?select=*", headers=headers)
            ...
            for r in rooms:
                occupied = [
                    {"from": res.get("startDate"), "to": res.get("endDate")} 
                    for res in reservations if res.get("roomId") == r.get("id")
                ]
                
                bot_payload["rooms"].append({
                    "id": r.get("id"),
                    "number": r.get("number"),
                    "category": r.get("category"),
                    "capacity": r.get("capacity"),
                    "prices": {
                        "nightly": r.get("basePrice", 0),
                        "weekend": r.get("weekendPrice") or r.get("basePrice", 0) * 1.2,
                        "january_week": r.get("januaryPrice", 0),
                        "february_week": r.get("februaryPrice", 0),
                        "promo": r.get("promoPrice", 0)
                    },
                    "occupied": occupied
                })
